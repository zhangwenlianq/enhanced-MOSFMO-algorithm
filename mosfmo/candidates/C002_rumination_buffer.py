"""Candidate C002: one-shot SFMO with intermittent rumination-buffer sharing."""
...
def optimize(
    ...
    guides = np.asarray([_leader(archive, (x, f, g), rng) for _ in range(n)])
    ...
    while evaluator.remaining:
        ...
        for i in range(count):
            ...
            if not pending_valid[i] or constrained_better(
                tf[i], tg[i], pending_f[i], pending_g[i], weights[i], ideal
            ):
                pending_x[i], pending_f[i], pending_g[i] = trial[i], tf[i], tg[i]
    ...
    return _finish(
        ...
        {
            "candidate": "C002-rumination-buffer",
            "cycles": cycles,
